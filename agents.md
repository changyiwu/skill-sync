...
- [ ] 階段七：擴充 `sync-skills` 支援「來源資料夾 → 安裝名」對照表（目前改名安裝的技能完全不在覆蓋範圍，見〈已知限制〉）
...
**`sync-skills` 只處理「來源資料夾名＝安裝名」的技能。**安裝時會改名的技能（例：`claude-code-lazy-packs/skills/08-draw` → `~/.claude/skills/claude-draw`）完全不在覆蓋範圍——不會被同步，也不會被驗證，所以它們落後時**沒有任何機制會發出聲音**。

2026-07-31 全盤比對本機四家共 42 項技能時證實了這個缺口：同名的專案類 16 項全部一致，改名安裝的 lazy-pack 類卻有 7 項落後，其中三項是會實際壞掉的（路徑指向已廢目錄、相對路徑導致跨專案找不到腳本、舊世代安裝指引會叫人裝已移除的技能）。修復只能手動 `Copy-Item` 逐項改名。

根治方式是階段七：讓技能接受「來源資料夾 → 安裝名」對照表，改名安裝的技能才能一併納入同步與 hash 驗證。
