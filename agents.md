...
- [ ] 階段九：修掉階段八盤點抓到的三個複製缺陷（占位符替換型安裝、專案根目錄即技能、衍生物排除）
...
- **占位符替換型安裝會被覆蓋壞掉（階段九待修）**。`html-slide-builder` 的來源寫 `{{DRAW_SKILL_PATH}}`，安裝時每家替換成各自的生圖技能，所以四家副本兩兩不同、也都不等於來源。現行邏輯會拿範本蓋掉它們，把占位符寫死進去、四個技能一起壞。技能得先偵測到這種模式並停下來。
- **專案根目錄即技能時會複製到專案基礎建設（階段九待修）**。`SKILL.md` 放在 repo 根目錄的話，`.git/`、`agents.md`、`handoff.md`、`README.md`、`LICENSE` 全都落在複製範圍內。2026-08-01 在 `rdq-skill` 實測到這個情況（四家副本各約 38 個檔對不上），**已從源頭解決**——該 repo 把 Skill 本體 `git mv` 進 `rdq/` 子資料夾，四家立刻變成 `OK`（3 檔）。目前本機沒有其他根目錄型的技能 repo，但**技能本身的缺陷未修**：只要再出現一個，同樣會踩。根治方向是複製前把「專案基礎建設」列出來給使用者確認，而不是預設整包複製。
- **衍生物沒排除在複製與比對外（階段九待修）**。`__pycache__` 已排除於技能**偵測**，但沒排除於**複製與 hash 比對**，所以 `html-slide-builder` 每次都會報 `.pyc` 不一致——那是每台機器自己產生的，永遠不可能相同。
